...
class ShipClassifier2ConvNet(nn.Module):
    ...
    def __init__(self, dropout: float = 0.2) -> None:
        """Initialise the ShipClassifier2ConvNet class. The model consists of
        ...
        self.fc_layers = nn.Sequential(
            ...
            ),  # Output 2 classes (ship or no ship)
        )
...
class ShipClassifier4ConvNet(nn.Module):
    ...
    def __init__(self, dropout: float = 0.2) -> None:
        """Initialise the ShipClassifier4ConvNet class. The model consists of
...
def load_model_from_hf(
    ...
    try:
        ...
        model.load_state_dict(
            torch.load(f=model_path, map_location=device, weights_only=weights_only)
        )
    except Exception as e:
...
def visualise_feature_maps(
    ...
    def hook_fn(
        ...
        """Hook function to capture the output (feature maps) of a layer during
        a forward pass. Used as a forward hook to intercept the output of a specific
